...
public class DapperContext(IDapperSettings dapperSettings) : IDapperContext, IDisposable
{
    ...
    public async Task<T?> FirstOrDefault<T>(IQueryObject queryObject)
    {
        return await Execute(query =>
                query.QueryFirstOrDefaultAsync<T>(queryObject.Sql, queryObject.Parameters))
    ...
    {
        return (await Execute(query =>
                    query.QueryAsync<T>(queryObject.Sql, queryObject.Parameters))
                .ConfigureAwait(false))
            .AsList();
    }
    ...
    {
        return await Execute(query =>
                query.QueryFirstAsync<T>(queryObject.Sql, queryObject.Parameters,
    ...
    {
        await Execute(query =>
                query.ExecuteAsync(queryObject.Sql, queryObject.Parameters,
                    transaction: _transaction))
            .ConfigureAwait(false);
    }
